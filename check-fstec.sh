...
echo "Скрипт проверки соответствия настройки Linux"

# Методический документ от 25 декабря 2022 г.
...
echo "Методический документ от 25 декабря 2022 г."
echo "Рекомендации по безопасной настройке операционных систем Linux"
echo "https://fstec.ru/dokumenty/vse-dokumenty/spetsialnye-normativnye-dokumenty/metodicheskij-dokument-ot-25-dekabrya-2022-g"
echo ""

# 2.1. Настройка авторизации в операционных системах Linux
echo "2.1. Настройка авторизации в операционных системах Linux"
echo ""

# 2.1.1. Не допускать использование учетных записей пользователей с пустыми паролями. Настроить учетные записи таким образом,
# чтобы каждый пользователь системы либо имел пароль, либо был заблокирован по паролю. В системах Linux данную возможность обеспечивает
# файл /etc/shadow.
echo "2.1.1. Не допускать использование учетных записей пользователей с пустыми паролями. Настроить учетные записи таким образом, чтобы каждый пользователь системы либо имел пароль, либо был заблокирован по паролю. В системах Linux данную возможность обеспечивает файл /etc/shadow."
echo ""

output=$(cat /etc/passwd | grep -v ":x:")
if [ -n "$output" ]; then
  echo "FAIL: В файле /etc/passwd есть пользователи, пароли которых не содержатся в /etc/shadow"
  echo $output
else
  echo "PASS: В файле /etc/passwd нет аномалий"
fi
echo ""

# 2.1.2. Обеспечить отключение входа суперпользователя в систему по протоколу SSH путём установки для параметра PermitRootLogin значения
# no в файле /etc/ssh/sshd_config.
echo "2.1.2. Обеспечить отключение входа суперпользователя в систему по протоколу SSH путём установки для параметра PermitRootLogin значения no в файле /etc/ssh/sshd_config."
echo ""

output=$(sudo cat /etc/ssh/sshd_config | grep -v "#"  | grep "PermitRootLogin" |  grep "no" )
if [ -n "$output" ]; then
  echo "PASS В файле /etc/ssh/sshd_config установлен запрет на аутентификацю от имени root"
  echo $output
else
  echo "FAIL"
fi
...
echo " 2.2.1. Обеспечить ограничение доступа к команде su путём добавления в файл /etc/pam.d/su следующей строки: auth required pam_wheel.so use_uid. Задать список пользователей в записи для группы wheel в файле /etc/group:"
echo "wheel:x:10:root,<user list>."
echo ""

output=$(sudo cat /etc/pam.d/su | grep -v "#"  | grep "auth required pam_wheel.so use_uid")
if [ -n "$output" ]; then
  echo "PASS В файле /etc/pam.d/su найдена строка auth required pam_wheel.so use_uid"
  echo $output
  echo "WARNING Проверьте, что список пользователей, которым разрешён запуск su адекватен"
  sudo cat /etc/group | grep "wheel:"
  sudo cat /etc/group | grep "sudo:"
else
  echo "FAIL В файле /etc/pam.d/su не найдена строка auth required pam_wheel.so use_uid"
fi
echo ""

# 2.2.2. Ограничить список пользователей, которым разрешено использовать команду sudo и разрешенных к выполнению через sudo команд путём пересмотра
# файла /etc/sudoers.
echo "2.2.2. Ограничить список пользователей, которым разрешено использовать команду sudo и разрешенных к выполнению через sudo команд путём пересмотра файла /etc/sudoers."
echo ""

echo "WARNING Ниже содержание файла /etc/sudoers, проверьте самостоятельно"
...
# (/etc/passwd) и групп (/etc/group), либо хранилищам хешей паролей
...
echo "2.3.1. Установить корректные права доступа к файлам настройки пользователей, а именно к файлам с перечнями пользовательских идентификаторов (/etc/passwd) и групп (/etc/group), либо хранилищам хешей паролей (в операционных системах GNU/Linux, Solaris, HP-UX: /etc/shadow, AIX: /etc/security/passwd), с помощью команд:"
echo "chmod 644 /etc/passwd;"
echo "chmod 644 /etc/group;"
echo "chmod go-rwx /etc/shadow."
echo ""

echo "WARNING Настройки применятся автоматически"
...
# 2.3.2. Установить корректные права доступа к файлам запущенных процессов путём выполнения команды вида: chmod go-w /путь/к/файлу для всех
# исполняемых файлов, запущенных в настоящий момент, и соответствующих библиотек. После этого необходимо осуществить проверку, что директория,
# содержащая данный файл, а также все родительские директории недоступны для записи непривилегированным пользователям.
echo "2.3.2. Установить корректные права доступа к файлам запущенных процессов путём выполнения команды вида: chmod go-w /путь/к/файлу для всех исполняемых файлов, запущенных в настоящий момент, и соответствующих библиотек. После этого необходимо осуществить проверку, что директория, содержащая данный файл, а также все родительские директории недоступны для записи непривилегированным пользователям."
echo ""
echo "WARNING Изучить запущенные процессы самостоятельно"
echo ""
...
# 2.3.10. Установить корректные права доступа к содержимому домашних директорий пользователей (.bash_history, .history, .sh_history и т. п. - файлы истории команд оболочек, .bash_profile, .bashrc, .profile, .bash_logout и т. п. - файлы настройки оболочки, .rhosts - настройки R-подсистем) путём установки на каждый из указанных файлов корректных прав доступа с помощью команды вида: chmod go-rwx путь_к_файлу.
...
echo "Настройка механизмов защиты ядра Linux"
...
# 2.4.6. Рандомизировать расположение ядерного стека путём установки значения опции загрузки ядра randomize_kstack_offset=1. Эта настройка позволяет изменить значение опции сборки ядра CONFIG_RANDOMIZE_KSTACK_OFFSET_
# DEFAULT при запуске системы (per boot).
...
echo "Уменьшение периметра атаки ядра Linux"
...
echo "Настройка средств защиты пользовательского пространства со стороны ядра Linux"
